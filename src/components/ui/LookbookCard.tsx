...
export function LookbookCard({ item, variant = 'horizontal' }: LookbookCardProps) {
  const { t } = useLanguage()
  const isReserved = !item.image
  ...
  return (
    <figure
      className={`lookbook-item group relative shrink-0 overflow-hidden rounded-md bg-zinc-900 ${horizontalCardClass}`}
    >
